...
# Bounded cache of recently seen product ids. Using a deque(maxlen=N) keeps the
# tracking structure bounded so sustained load cannot grow the working set
# without limit (fixes the unbounded list that leaked and OOM-killed the pod).
...
def get_recommendations(input_product_ids: list[str], max_results: int = 5) -> list[str]:
    ...
    _request_count += 1
    # track recently seen ids into the bounded cache
...
def main() -> None:
    ...
    print(f"[recommendation] serving on :{port} (bounded cache)", flush=True)
    srv.serve_forever()
