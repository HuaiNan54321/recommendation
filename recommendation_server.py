...
# Module-level cache of recently seen ids. MUST stay bounded: every
# GetRecommendations call appends to it, so an unbounded container grows the
# working set without limit under sustained load -> OOM.
...
def get_recommendations(input_product_ids: list[str], max_results: int = 5) -> list[str]:
    ...
    _request_count += 1
    # record recently seen ids; deque(maxlen=...) evicts the oldest entries so
    # this never grows beyond _SEEN_IDS_MAXLEN
